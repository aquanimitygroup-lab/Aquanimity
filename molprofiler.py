...
from interaction_analyzer import analyze_interactions, format_interactions_for_csv
import adme_pred
                ...
                if score is not None:
                    ...
                    try:
                        ...
                        # Get ligand PDB content
                                                # AFTER (working):
                        # Sanitize ligand to fix valence issues
                        ligand_copy = Chem.Mol(info['ligand'])
                        try:
                            Chem.SanitizeMol(ligand_copy, catchErrors=True)
                        except:
                            # If sanitization fails, try removing/re-adding hydrogens
                        ...
                        interaction_result = analyze_interactions(
                            info['ligand'], 
                            pdb_path  # already defined on line 262
                        )
